    	...
    	table: {
    		...
    		exportExcel: function(formId) {
    			$.modal.confirm("确定导出所有" + $.table._option.modalName + "吗？", function() {
					window.location.href ="/sys/warning/export";
    			});
    		},
    		// 下载模板
    		importTemplate: function() {
    			$.get($.table._option.importTemplateUrl, function(result) {
    				if (result.code == web_status.SUCCESS) {
    			        window.location.href = "/sys/sensor/common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
        ...
        operate: {
			...
			editSelect: function() {
				...
				$.modal.confirm("<span>处理说明：</span style='float: left;'><textarea id='warning_dis' maxlength='200' style='margin: 0px; height: 85px; width: 263px;'></textarea>" +
					"<br>确定将选中的<span style='color: #ED5565;'>" + rows.length + "条</span>数据置为已处理状态吗？", function() {
					var url = $.table._option.batchUpdateUrl;
					var warning_dis = $("#warning_dis").val();
					var data = { "ids": rows.join(),"warningDis":warning_dis};
					$.operate.submit(url, "post", "json", data);
				});
			},
            // 修改信息，以tab页展现
